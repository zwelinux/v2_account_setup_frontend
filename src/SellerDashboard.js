...
function SellerDashboard() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const res = await fetch('https://ladyfirstme.pythonanywhere.com/api/auth/seller/orders/', {
        headers: {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    const token = localStorage.getItem('access_token');
    try {
      const res = await fetch(`https://ladyfirstme.pythonanywhere.com/api/auth/seller/orders/${orderId}/update-status/`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
